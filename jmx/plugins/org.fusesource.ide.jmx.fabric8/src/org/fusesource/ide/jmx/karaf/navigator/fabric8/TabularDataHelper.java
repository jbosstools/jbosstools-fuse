			...
			if (tabularData != null) {
				...
				for (Object row : rows) {
					if (row instanceof CompositeData) {
						...
						Fabric8JMXPlugin.getLogger().debug("===== uknown row type: " + row + " of type " + Objects.typeName(row));
					}
				}
			}
		} catch (Exception e) {
			Fabric8JMXPlugin.getLogger().error("Failed to convert TabularData to List<IPropertySource>: " + e, e);
		}
		return answer;
	}
}
