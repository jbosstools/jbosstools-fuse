    ...
    public IContextButtonPadData getContextButtonPad(IPictogramElementContext context) {
        ...
        for (ICustomFeature f : cf) {
            /**if (f instanceof DeleteAllEndpointBreakpointsFeature) {
                continue;
            } else if (f instanceof SetEndpointBreakpointFeature) {
            ...
            } else **/if (f instanceof CollapseFeature && f.canExecute(cc)) {
            	String image = IPlatformImageConstants.IMG_EDIT_COLLAPSE;
            	String collapseExpand = "Collapse";
            	
            	if (Boolean.parseBoolean(Graphiti.getPeService().getPropertyValue(pe, CollapseFeature.PROP_COLLAPSED_STATE))) {
...
//        	} else if (f instanceof LayoutDiagramFeature && bo instanceof CamelModelElement && ((CamelModelElement)bo).getUnderlyingMetaModelObject().canHaveChildren()) {
//        		IContextButtonEntry layoutButton = new ContextButtonEntry(f, cc);
//        		layoutButton.setDescription("Layout");
//        		layoutButton.setText("Layout");
//        		layoutButton.setIconId(ImageProvider.IMG_REDDOT);
//            	data.getDomainSpecificContextButtons().add(layoutButton);
        	}
        }
