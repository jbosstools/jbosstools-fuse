...
public class PreferredLabelDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		model.getEipModel().getSupportedEIPs().stream().map(eip -> eip.getName()).sorted().forEach(componentNames::add);
	...
	private void setParameterItems(CamelModel model) {
		...
		if (!componentName.isEmpty()) {
			List<Parameter> params = model.getEipModel().getEIPByName(componentName).getParameters();
