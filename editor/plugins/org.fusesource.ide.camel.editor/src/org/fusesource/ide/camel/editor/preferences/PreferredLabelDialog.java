...
public class PreferredLabelDialog extends TitleAreaDialog {
	...
	protected EipModel getEipModel() {
		if (eipModel == null) {
			eipModel = CamelModelFactory.getModelForVersion(CamelModelFactory.getLatestCamelVersion()).getEipModel();
		}
		return eipModel;
	}

	protected List<Eip> getComponents() {
		return getEipModel().getSupportedEIPs().stream().filter(eip -> !"NONE".equals(getCategoryFromEip(eip)))
	...
	public void create() {
		...
		if (component != null && !component.isEmpty()) {
			Eip eip = getEipModel().getEIPByName(component);
