 ...
 * @since 3.0
 */
public class RiderDesignEditorFlyoutPaletteComposite extends FlyoutPaletteComposite {
	 ...
	 */
	public RiderDesignEditorFlyoutPaletteComposite(Composite parent, int style,
	...
	private class ResizeAction extends Action {
		...
		public void run() {
			final Tracker tracker = new Tracker(RiderDesignEditorFlyoutPaletteComposite.this,
					SWT.RIGHT | SWT.LEFT);
	...
	private class TitleDragManager extends MouseAdapter implements Listener,
		...
		public void handleEvent(Event event) {
			...
			threshold = dock == PositionConstants.EAST ? Integer.MAX_VALUE / 2
					: -1;
			final Composite flyout = RiderDesignEditorFlyoutPaletteComposite.this;
						...
						public void run() {
							...
							if (!switchDock)
								invalid = !isDescendantOf(
										RiderDesignEditorFlyoutPaletteComposite.this, ctrl);
							if (switchDock) {
								if (dock == PositionConstants.WEST) {
	...
	private class TitleCanvas extends Canvas {
		...
		public void setFont(Font font) {
			...
			if (isVisible()) {
				 ...
				 * getParent().layout().
				 */
				RiderDesignEditorFlyoutPaletteComposite.this.layout(true);
