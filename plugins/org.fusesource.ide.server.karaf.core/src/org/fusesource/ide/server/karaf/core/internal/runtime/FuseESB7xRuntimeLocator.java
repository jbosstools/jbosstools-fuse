 ...
 * @author lhein
 */
public class FuseESB7xRuntimeLocator extends KarafRuntimeLocator {
	/**
	...
	public static IRuntimeWorkingCopy getRuntimeFromDir(File dir, IProgressMonitor monitor) {
		for (int i = 0; i < IKarafRuntime.FUSEESB_RUNTIME_TYPES_SUPPORTED.length; i++) {
			try {
				IRuntimeType runtimeType = ServerCore.findRuntimeType(IKarafRuntime.FUSEESB_RUNTIME_TYPES_SUPPORTED[i]);
				String absolutePath = dir.getAbsolutePath();
				
				// now check if the directory is valid
				if (FuseESBUtils.isValidFuseESBInstallation(dir, null)) {
