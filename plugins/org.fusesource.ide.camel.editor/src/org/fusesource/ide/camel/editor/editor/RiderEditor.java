...
ITabbedPropertySheetPageContributor, IPrefersPerspective, IPropertyChangeListener {
	...
	/**
	 * We are recreating the design page due to an underlying model change
	 */
	public void recreateDesignPage() {
		if (designEditor != null) {

			// NOTE that calling removePage() tends to bork the tabs up in the
			// UI
			// so lets manually delete the editor by hand...
			/*
			 * 
			 * removePage(designPageIndex); createDesignPage(designPageIndex);
			 */

			int pageIndex = designPageIndex;
			Assert.isTrue(pageIndex >= 0 && pageIndex < getPageCount());

			// lets make sure we flush any commands first
			designEditor.getCommandStack().flush();

			// use reflection to get the private stuff... - dirty but what else
			// can we do :(
			CTabFolder tabFolder = (CTabFolder) getField(this, "container", MultiPageEditorPart.class); // getTabFolder();
			ArrayList nestedEditors = (ArrayList) getField(this, "nestedEditors", MultiPageEditorPart.class);
			if (nestedEditors == null) {
				nestedEditors = new ArrayList();
			}

			// TODO doing this zaps the serviceLocator for the tabs...
			// disposePart(designEditor);
			designEditor.dispose();
			// lets install a dummy command stack to prevent NPE
			designEditor.getEditDomain().setCommandStack(new CommandStack());
			designEditor.getParent().dispose();

			nestedEditors.remove(designEditor);

			designEditor = new RiderDesignEditor(this);
			IEditorPart editor = designEditor;
			...
			// call init first so that if an exception is thrown, we have
			// created no
			// new widgets
			try {
				editor.init(site, editorInput);

				Composite parent2 = new Composite(getContainer(), designEditor.getOrientation());
				...
				// update the tab item
				if (tabFolder != null) {
					Item item = tabFolder.getItem(designPageIndex);
					if (item != null) {
						item.setData(designEditor);
						((CTabItem) item).setControl(parent2);
					}
				}
			} catch (PartInitException e) {
				Activator.getLogger().warning("Failed to recreate DiagramEditor: " + e, e);
			}
		}
	}
