...
public class NodeTable extends TableViewSupport implements IPropertySheetPage {
	...
	public NodeTable(CloudNode node) {
		this.node = node;
	}

	public ComputeService getComputeService() {
		return node.getComputeService();
	}
				...
				if (!selectedNodes.isEmpty()) {
					...
					if (confirm) {
						Jobs.schedule(makeNodesString(selectedNodes, "Destroying ", ""), new Runnable() {
							@Override
							public void run() {
								foreachNode(selectedNodes, "Destroy node failed", new Function<ComputeMetadata, Void>() {
	...
	/*
	public void addCloud(ComputeMetadata details) {
		if (!ComputeMetadata.getCloudDetailList().contains(details)) {
			details.flush();
			ComputeMetadata.getCloudDetailList().add(details);
			getViewer().setSelection(new StructuredSelection(details));
		}
		getViewer().refresh();
	}

	public void removeComputeMetadata(ComputeMetadata details) {
		if (details != null) {
			details.delete();
			ComputeMetadata.getCloudDetailList().remove(details);
			getViewer().refresh();
		}
	}

	protected void editCloud(final ComputeMetadata cloudDetails) {
		cloudDetails.flush();
		try {
			reload();
		} catch (Exception e) {
		...
		Object found = Iterables.find(ComputeMetadata.getCloudDetailList(), new Predicate<ComputeMetadata>(){

			@Override
			public boolean apply(ComputeMetadata details) {
