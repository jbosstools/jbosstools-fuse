...
find * -name 'pom.xml' | xargs perl -pi -e "s/<version>7.1.1-SNAPSHOT<\/version>/<version>${VERSION}<\/version>/g"

# replace the OSGi version names in poms
find plugins -name 'pom.xml' | xargs perl -pi -e "s/<version>7.1.1.qualifier<\/version>/<version>${VERSION}<\/version>/g"

# replace manifest versions
find plugins -name '*.xml' | xargs perl -pi -e "s/7.1.1.qualifier/${VERSION}/g"
