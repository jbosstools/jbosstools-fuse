...
public class DownloadServerTest extends DefaultTest {
	...
	/*
	 * Set credentials which are required to download some runtimes.
	 */
	@BeforeClass
	public static void setCredentials() {
		...
		new WaitUntil(new ControlIsEnabled(new PushButton(OK_BUTTON)));
		new PushButton(OK_BUTTON).click();
	...
	public void testDownloadServerRuntimeTest() {
		 ...
		 */
		if (parLoginRequired) {
			new WaitUntil(new ControlIsEnabled(new PushButton(NEXT_BUTTON)));
			assertTrue(buttonNext.isEnabled());
			buttonNext.click();
		...
		AbstractWait.sleep(TimePeriod.getCustom(2));
		new WaitUntil(new ControlIsEnabled(new PushButton(FINISH_BUTTON)));
		new PushButton(FINISH_BUTTON).click();
