...
public class CamelService implements ICamelManagerService {
	 ...
	 */
	private static final String PATTERN_BUNDLE_ENTRIES = "%s/%s/%s";
	...
	private void getCatalogInstance() {
		if (catalog == null) {
	...
	private URL getComponentModelURL(String runtimeProvider) {
		return CamelServiceImplementationActivator.getDefault().getBundle().getEntry(String.format(PATTERN_BUNDLE_ENTRIES, CATALOG_FOLDER, runtimeProvider, COMPONENTS_FILENAME));
	}
	
	private URL getEipModelURL(String runtimeProvider) {
		return CamelServiceImplementationActivator.getDefault().getBundle().getEntry(String.format(PATTERN_BUNDLE_ENTRIES, CATALOG_FOLDER, runtimeProvider, EIPS_FILENAME));
	}
	
	private URL getDataFormatModelURL(String runtimeProvider) {
		return CamelServiceImplementationActivator.getDefault().getBundle().getEntry(String.format(PATTERN_BUNDLE_ENTRIES, CATALOG_FOLDER, runtimeProvider, DATAFORMATS_FILENAME));
	}
	
	private URL getLanguageModelURL(String runtimeProvider) {
		return CamelServiceImplementationActivator.getDefault().getBundle().getEntry(String.format(PATTERN_BUNDLE_ENTRIES, CATALOG_FOLDER, runtimeProvider, LANGUAGES_FILENAME));
	}
