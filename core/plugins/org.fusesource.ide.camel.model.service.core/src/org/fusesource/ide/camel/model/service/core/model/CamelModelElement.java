...
public class CamelModelElement {
	...
	public CamelModelElement(CamelModelElement parent, Node underlyingNode) {
		...
		System.err.println("[" + hashCode() + "] Node " + (xmlNode != null ? xmlNode.getNodeName() : "null") + ", Parent: " + (parent != null && parent.getUnderlyingMetaModelObject() != null ? parent.getUnderlyingMetaModelObject().getName() : "null") + ", Input: " + (getInputElement() != null ? getInputElement().hashCode() : "null") + ", Output: " + (getOutputElement() != null ? getOutputElement().hashCode() : "null"));
	}
	...
	protected Eip getEipByName(String name) {
		...
		String prjCamelVersion = CamelModelFactory.getCamelVersion(prj);
	 ...
	 * @return
	 */
	protected boolean hasEipOutputs(String name) {
		Eip eip = getEipByName(name);
		if (eip != null) {
			boolean hasOutputsParam = false;
			Iterator<Parameter> it = eip.getParameters().iterator();
			while (it.hasNext()) {
				Parameter p = it.next();
				if (p.getName().equalsIgnoreCase("outputs")) {
					hasOutputsParam = true;
					break;
				}
			}
			return eip.getOutput().equalsIgnoreCase(Boolean.TRUE.toString()) && hasOutputsParam;
		}
		return false;
	}
	...
	protected void parseChildren() {
		...
		boolean hasChildren = hasEipOutputs(nodeName) || hasSpecialHandling(nodeName);
		NodeList children = getXmlNode().getChildNodes();
		CamelModelElement lastNode = this;
		for (int i=0; i<children.getLength(); i++) {
			Node tmp = children.item(i);
			if (tmp.getNodeName().equals("#text")) continue;
			CamelModelElement cme = new CamelModelElement(this, tmp);
			boolean isUnconnectedEip = isUnconnectedEip(cme);
			if (hasChildren || isUnconnectedEip) {
				addChildElement(cme);
			} else {
				cme.setParent(getParent());
				setOutputElement(cme);
			}
	...
	/**
	 * checks if the Eip has both input and output set to false
	 * 
	 * @param cme
	 * @return
	 */
	protected boolean isUnconnectedEip(CamelModelElement cme) {
		return cme.getUnderlyingMetaModelObject().getInput().equalsIgnoreCase("false") &&
			   cme.getUnderlyingMetaModelObject().getOutput().equalsIgnoreCase("false");
	...
	/**
	 * get the camel file object 
	 * 
	 * @return
	 */
	protected CamelFile getCamelFile() {
		if (getParent() != null) {
			if (getParent() instanceof CamelFile) {
				return (CamelFile)getParent();
			} else {
				return getParent().getCamelFile();
			}			
		} else {
			return (CamelFile)this;
		}
	}
    ...
    public String getIconName() {
    	return String.format("%s", getNodeTypeId());
    }
    ...
    public String getCategoryName() {
    	// TODO: find a better logic for handling the tags / categories
    	return this.underlyingMetaModelObject.getTags().get(this.underlyingMetaModelObject.getTags().size()-1);
    }
}
